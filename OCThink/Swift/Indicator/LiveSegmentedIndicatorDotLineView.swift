//
//  LiveSegmentedIndicatorDotLineView.swift
//  LiveSegmentedView
//
...
open class LiveSegmentedIndicatorDotLineView: LiveSegmentedIndicatorBaseView {
    /// 线的最大宽度
    ...
    open override func refreshIndicatorState(model: LiveSegmentedIndicatorSelectedParams) {
    ...
    open override func contentScrollViewDidScroll(model: LiveSegmentedIndicatorTransitionParams) {
        ...
        if percent <= 0.5 {
            targetX = LiveSegmentedViewTool.interpolate(from: leftX, to: centerX, percent: CGFloat(percent*2))
            targetWidth = LiveSegmentedViewTool.interpolate(from: dotWidth, to: lineMaxWidth, percent: CGFloat(percent*2))
        }else {
            targetX = LiveSegmentedViewTool.interpolate(from: centerX, to: rightX, percent: CGFloat((percent - 0.5)*2))
            targetWidth = LiveSegmentedViewTool.interpolate(from: lineMaxWidth, to: dotWidth, percent: CGFloat((percent - 0.5)*2))
        }
    ...
    open override func selectItem(model: LiveSegmentedIndicatorSelectedParams) {
