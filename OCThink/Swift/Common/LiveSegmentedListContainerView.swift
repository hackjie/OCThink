//
//  LiveSegmentedListContainerView.swift
//  LiveSegmentedView
//
...
public enum LiveSegmentedListContainerType {
    case scrollView
    case collectionView
}

@objc
public protocol LiveSegmentedListContainerViewListDelegate {
    /// 如果列表是VC，就返回VC.view
    /// 如果列表是View，就返回View自己
    ///
    /// - Returns: 返回列表视图
...
@objc
public protocol LiveSegmentedListContainerViewDataSource {
    /// 返回list的数量
    func numberOfLists(in listContainerView: LiveSegmentedListContainerView) -> Int

    /// 根据index初始化一个对应列表实例，需要是遵从`LiveSegmentedListContainerViewListDelegate`协议的对象。
    /// 如果列表是用自定义UIView封装的，就让自定义UIView遵从`LiveSegmentedListContainerViewListDelegate`协议，该方法返回自定义UIView即可。
    /// 如果列表是用自定义UIViewController封装的，就让自定义UIViewController遵从`LiveSegmentedListContainerViewListDelegate`协议，该方法返回自定义UIViewController即可。
    /// 注意：一定要是新生成的实例！！！
    ///
    /// - Parameters:
    ///   - listContainerView: LiveSegmentedListContainerView
    ///   - index: 目标index
    /// - Returns: 遵从LiveSegmentedListContainerViewListDelegate协议的实例
    func listContainerView(_ listContainerView: LiveSegmentedListContainerView, initListAt index: Int) -> LiveSegmentedListContainerViewListDelegate


    /// 控制能否初始化对应index的列表。有些业务需求，需要在某些情况才允许初始化某些列表，通过通过该代理实现控制。
    @objc optional func listContainerView(_ listContainerView: LiveSegmentedListContainerView, canInitListAt index: Int) -> Bool

    /// 返回自定义UIScrollView或UICollectionView的Class
    /// 某些特殊情况需要自己处理UIScrollView内部逻辑。比如项目用了FDFullscreenPopGesture，需要处理手势相关代理。
    ///
    /// - Parameter listContainerView: LiveSegmentedListContainerView
    /// - Returns: 自定义UIScrollView实例
    @objc optional func scrollViewClass(in listContainerView: LiveSegmentedListContainerView) -> AnyClass
}

open class LiveSegmentedListContainerView: UIView, LiveSegmentedViewListContainer, LiveSegmentedViewRTLCompatible {
    open private(set) var type: LiveSegmentedListContainerType
    open private(set) weak var dataSource: LiveSegmentedListContainerViewDataSource?
    open private(set) var scrollView: UIScrollView!
    /// 已经加载过的列表字典。key是index，value是对应的列表
    open private(set) var validListDict = [Int:LiveSegmentedListContainerViewListDelegate]()
    ...
    private lazy var containerVC = LiveSegmentedListContainerViewController()
    private var willAppearIndex: Int = -1
    private var willDisappearIndex: Int = -1

    public init(dataSource: LiveSegmentedListContainerViewDataSource, type: LiveSegmentedListContainerType = .scrollView) {
    ...
    open func commonInit() {
        ...
        if type == .scrollView {
            ...
            collectionView.register(LiveSegmentedRTLCollectionCell.self, forCellWithReuseIdentifier: "cell")
    ...
    //MARK: - LiveSegmentedViewListContainer
...
extension LiveSegmentedListContainerView: UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
...
class LiveSegmentedListContainerViewController: UIViewController {
