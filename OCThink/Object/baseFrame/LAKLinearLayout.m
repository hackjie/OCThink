...
@implementation LAKLinearLayout
...
- (instancetype)initWithFrame:(CGRect)frame orientation:(LAKOrientation)orientation {
    ...
    if (self) {
        LAKLinearLayoutTraits *layoutTraits = (LAKLinearLayoutTraits*)self.myDefaultSizeClass;
        if (orientation == LAKOrientation_Vert) {
            [layoutTraits.heightSize _myEqualTo:@(LAKLayoutSize.wrap) priority:MyPriority_Low]; //这是暂时先设置为低优先级,为了兼容老版本。
        } else {
            [layoutTraits.widthSize _myEqualTo:@(LAKLayoutSize.wrap) priority:MyPriority_Low];
        }
...
- (void)setOrientation:(LAKOrientation)orientation {
    LAKLinearLayoutTraits *layoutTraits = (LAKLinearLayoutTraits*)self.myDefaultSizeClass;
...
- (LAKOrientation)orientation {
    return self.myDefaultSizeClassInner.orientation;
}

- (void)setShrinkType:(LAKSubviewsShrinkType)shrinkType {
    LAKLinearLayoutTraits *layoutTraits = (LAKLinearLayoutTraits*)self.myDefaultSizeClass;
...
- (LAKSubviewsShrinkType)shrinkType {
...
- (CGSize)calcLayoutSize:(CGSize)size subviewEngines:(NSMutableArray<LAKLayoutEngine *> *)subviewEngines context:(MyLayoutContext *)context {
    ...
    LAKLinearLayoutTraits *layoutTraits = (LAKLinearLayoutTraits *)context->layoutViewEngine.currentSizeClass;
    ...
    LAKOrientation orientation = layoutTraits.orientation;
    
    [self myCalcSubviewsWrapContentSize:context
                      withCustomSetting:^(LAKViewTraits *subviewTraits) {
                          [self myLayout:layoutTraits orientation:orientation gravity:(orientation == LAKOrientation_Vert) ? context->horzGravity : context->vertGravity adjustSizeSettingOfSubview:subviewTraits];
                      }];
...
- (id)createSizeClassInstance {
    return [LAKLinearLayoutTraits new];
}

#pragma mark-- Private Methods

//调整子视图的wrapContent设置
- (void)myLayout:(LAKLinearLayoutTraits *)layoutTraits orientation:(LAKOrientation)orientation gravity:(LAKGravity)gravity adjustSizeSettingOfSubview:(LAKViewTraits *)subviewTraits {
    if (orientation == LAKOrientation_Vert) {
        //如果是拉伸处理则需要把包裹宽度取消。
        if (subviewTraits.widthSizeInner.wrapVal && gravity == LAKGravity_Horz_Fill) {
        ...
        //如果是拉伸处理则需要把包裹高度
        if (subviewTraits.heightSizeInner.wrapVal && gravity == LAKGravity_Vert_Fill) {
            [subviewTraits.heightSizeInner _myEqualTo:nil];
        }
...
- (void)myDoVertOrientationLayoutWithContext:(MyLayoutContext *)context {
    ...
    //高度可以被伸缩的子视图集合
    NSMutableSet<LAKLayoutEngine *> *flexHeightSubviewEngines = [NSMutableSet new];
    //高度是固定尺寸的子视图集合
    NSMutableArray<LAKLayoutEngine*> *fixedHeightSubviewEngines = [NSMutableArray new];
    ...
    for (LAKLayoutEngine *subviewEngine in context->subviewEngines) {
        ...
        if (context->vertGravity == LAKGravity_Vert_Fill || context->vertGravity == LAKGravity_Vert_Stretch) {
            ...
            //布局视图的高度自适应尺寸的不能被伸缩。
            if (subviewTraits.heightSizeInner.wrapVal && [subviewTraits.view isKindOfClass:[LAKBaseLayout class]]) {
                isFlexEngine = NO;
            }
            //高度最小是自身尺寸的不能被伸缩。
            ...
            if (context->vertGravity == LAKGravity_Vert_Stretch &&
                subviewTraits.heightSizeInner.val != nil &&
                (!subviewTraits.heightSizeInner.wrapVal || [subviewTraits.view isKindOfClass:[LAKBaseLayout class]])) {
                isFlexEngine = NO;
            }
            if (isFlexEngine) {
    ...
    LAKSubviewsShrinkType sstMode = layoutTraits.shrinkType & 0x0F;    //压缩的模式
    LAKSubviewsShrinkType sstContent = layoutTraits.shrinkType & 0xF0; //压缩内容
            ...
            if (sstContent == MySubviewsShrink_Size) { //压缩尺寸
                if (fixedHeightSubviewEngines.count > 0 && spareHeight < 0 && context->selfSize.height > 0) {
                    if (sstMode == MySubviewsShrink_Average) { //均分。
                        ...
                        for (LAKLayoutEngine *fixedHeightSubviewEngine in fixedHeightSubviewEngines) {
    ...
    //如果有浮动尺寸或者有压缩模式
    if (totalWeight != 0.0 || totalShrink != 0.0 || (sstMode != MySubviewsShrink_None && _myCGFloatLessOrEqual(spareHeight, 0)) || context->vertGravity != LAKGravity_None || layoutTraits.widthSizeInner.wrapVal) {
        maxLayoutWidth = 0.0;
        CGFloat incSpacing = 0.0; //间距扩充
        CGFloat incHeight = 0.0;    //尺寸扩充
        if (context->vertGravity == LAKGravity_Vert_Center) {
            pos = (context->selfSize.height - totalHeight - paddingVert) / 2.0 + context->paddingTop;
        } else if (context->vertGravity == LAKGravity_Vert_Window_Center) {
            if (self.window != nil) {
        ...
        } else if (context->vertGravity == LAKGravity_Vert_Among) {
            incSpacing = (context->selfSize.height - totalHeight - paddingVert) / (context->subviewEngines.count + 1);
            pos = context->paddingTop + incSpacing;
        } else if (context->vertGravity == LAKGravity_Vert_Fill || context->vertGravity == LAKGravity_Vert_Stretch) {
            pos = context->paddingTop;
...
- (void)myDoHorzOrientationLayoutWithContext:(MyLayoutContext *)context {
    ...
    //宽度是可伸缩的子视图集合
    NSMutableSet<LAKLayoutEngine *> *flexWidthSubviewEngines = [NSMutableSet new];
    //宽度是固定尺寸的子视图集合
    NSMutableArray<LAKLayoutEngine *> *fixedWidthSubviewEngines = [NSMutableArray new];
    //左右拉伸宽度尺寸的子视图集合
    NSMutableArray<LAKLayoutEngine *> *leftRightFlexWidthSubviewEngines = [NSMutableArray new];
    ...
    for (LAKLayoutEngine *subviewEngine in context->subviewEngines) {
        ...
        LAKViewTraits *subviewTraits = subviewEngine.currentSizeClass;
        if (context->horzGravity == LAKGravity_Horz_Fill || context->horzGravity == LAKGravity_Horz_Stretch) {
            BOOL isFlexEngine = YES;
            //设置了比重宽度的子视图不伸缩
            if (subviewTraits.weight != 0.0) {
                isFlexEngine = NO;
            ...
            //布局子视图宽度自适应时不伸缩
            if (subviewTraits.widthSizeInner.wrapVal && [subviewTraits.view isKindOfClass:[LAKBaseLayout class]]) {
                isFlexEngine = NO;
            }
            //子视图宽度最小为自身宽度时不伸缩。
            ...
            if (context->horzGravity == LAKGravity_Horz_Stretch &&
                subviewTraits.widthSizeInner.val != nil &&
                (!subviewTraits.widthSizeInner.wrapVal || [subviewTraits.view isKindOfClass:[LAKBaseLayout class]])) {
                isFlexEngine = NO;
            }
            if (isFlexEngine) {
        ...
        if (context->vertGravity == LAKGravity_Vert_Baseline && baselinePos == CGFLOAT_MAX && self.baselineBaseView == subviewTraits.view) {
            ...
            if (sstContent == MySubviewsShrink_Size) {
                if (fixedWidthSubviewEngines.count > 0 && spareWidth < 0 && context->selfSize.width > 0) {
                    ...
                    if (sstMode == MySubviewsShrink_Average) {
                        ...
                        for (LAKLayoutEngine *fixedWidthSubviewEngine in fixedWidthSubviewEngines) {
                            fixedWidthSubviewEngine.width += averageWidth;
                        }
                    } else if (sstMode == MySubviewsShrink_Auto) {
                        LAKLayoutEngine *leadingViewEngine = leftRightFlexWidthSubviewEngines[0];
                        LAKLayoutEngine *trailingViewEngine = leftRightFlexWidthSubviewEngines[1];
    ...
    //如果有浮动尺寸或者有压缩模式
    if (totalWeight != 0.0 || totalShrink != 0.0 || (sstMode != MySubviewsShrink_None && _myCGFloatLessOrEqual(spareWidth, 0)) || context->horzGravity != LAKGravity_None || layoutTraits.heightSizeInner.wrapVal) {
        maxLayoutHeight = 0.0;
        CGFloat incSpacing = 0.0; //间距扩充
        CGFloat incWidth = 0.0;    //尺寸扩充
        if (context->horzGravity == LAKGravity_Horz_Center) {
            pos = (context->selfSize.width - totalWidth - paddingHorz) / 2.0 + context->paddingLeading;
        } else if (context->horzGravity == LAKGravity_Horz_Window_Center) {
            if (self.window != nil) {
        ...
        } else if (context->horzGravity == LAKGravity_Horz_Among) {
            incSpacing = (context->selfSize.width - totalWidth - paddingHorz) / (context->subviewEngines.count + 1);
            pos = context->paddingLeading + incSpacing;
        } else if (context->horzGravity == LAKGravity_Horz_Fill || context->horzGravity == LAKGravity_Horz_Stretch) {
            pos = context->paddingLeading;
        ...
        for (LAKLayoutEngine *subviewEngine in context->subviewEngines) {
            LAKViewTraits *subviewTraits = (LAKViewTraits *)subviewEngine.currentSizeClass;
...
- (CGFloat)mySubviewEngine:(LAKLayoutEngine *)subviewEngine calcLeadingTrailingWithContext:(MyLayoutContext *)context {
    LAKViewTraits *subviewTraits = subviewEngine.currentSizeClass;
...
- (CGFloat)mySubviewEngine:(LAKLayoutEngine *)subviewEngine baselinePos:(CGFloat)baselinePos calcTopBottomWithContext:(MyLayoutContext *)context  {
    LAKViewTraits *subviewTraits = subviewEngine.currentSizeClass;
    UIView *subview = subviewTraits.view;
    subviewEngine.height = [self myHeightSizeValueOfSubviewEngine:subviewEngine withContext:context];
...
- (void)myEqualizeSubviewEngines:(NSMutableArray<LAKLayoutEngine *> *)subviewEngines  orientation:(LAKOrientation)orientation centered:(BOOL)centered space:(CGFloat)space {
    ...
    for (LAKLayoutEngine *subviewEngine in subviewEngines) {
        LAKViewTraits *subviewTraits = subviewEngine.currentSizeClass;
        if (orientation == LAKOrientation_Vert) {
...
- (void)myEqualizeSubviewEngines:(NSArray<LAKLayoutEngine *> *)subviewEngines orientation:(LAKOrientation)orientation centered:(BOOL)centered {
    //如果居中和不居中则拆分出来的片段是不一样的。
